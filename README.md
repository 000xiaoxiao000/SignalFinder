...
- 基站距离估计：支持基于 RSRP 和本地基站点位库的加权最小二乘估计；点位不足时会明确显示缺失原因。
...
- 不能强制手机连接某个基站。
- 基站距离无法仅靠手机本地 API 直接计算。距离估计需要至少 3 个已知经纬度的基站点位，以及对应 RSRP 等测量数据，才能做基于信号强度和基站点位的加权最小二乘估计。
...
`referenceRsrpDbm`、`referenceDistanceMeters`、`pathLossExponent` 用于把 RSRP 近似换算成测距约束。没有实测校准时可以先使用默认值。找信号页面至少需要读到 3 个已配置点位的小区，才会输出“加权最小二乘”距离；不足 3 个时会显示“需至少3个有点位的小区”。
...
- 基站距离估计依赖 `cell_towers.json` 中的点位质量；点位不足或坐标误差较大时，距离只能作为粗略参考。
