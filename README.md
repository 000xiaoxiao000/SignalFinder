...
本项目仅用于合法的网络状态诊断、弱网排查、Android 权限与网络能力学习，以及用户对本人设备的本地调试。使用者应遵守所在地区适用的法律法规、运营商服务条款、平台规则和组织内部安全规范。

- 本项目不是代理、加速器、翻墙工具或网络监管规避工具。
- 本项目不提供访问境外受限网络服务的能力，也不应被改造成此类用途。
- 本项目不绕过 Android 权限模型，不静默提权，不隐藏执行系统命令。
- 使用者不得将本项目用于干扰网络秩序、规避合法网络管理措施、非法采集数据、侵犯他人隐私或其它违法违规用途。
- 本文档只说明项目技术边界和使用注意事项，不构成法律意见。如需正式判断，请咨询具备资质的专业人士。

## 隐私与数据处理

当前实现以本机诊断为主，不内置用户账号、远程后台或诊断数据上传逻辑。

- 网络检测会访问测试域名、测速地址和 DNS 服务器，用于计算延迟、丢包、下载速度和 DNS 响应时间。
- 小区信息、网络状态、应用列表、使用情况访问权限读取到的流量统计仅在本机展示和参与本机诊断。
- App 联网白名单基于本地 `VpnService` 丢弃未放行 App 的流量，不把流量转发到远程服务器。
- 应用日志用于本机排查问题；如对外提交日志，请先自行删除手机号、位置、应用列表、网络标识等敏感信息。
- 二维码图片保存到相册只在用户主动操作时发生。

## 禁止用途

请不要将本项目或其修改版用于以下场景：

- 规避网络监管、突破运营商或组织网络访问限制，或访问违法违规内容。
- 分发、交易、批量采集未经授权的基站坐标、地理位置数据、设备标识、应用使用记录或其它敏感数据。
- 添加隐藏上传、远程控制、静默提权、任意 Root 命令执行、代理转发、流量中继、绕过系统 VPN 提示等能力。
- 将修改版伪装成本项目官方版本发布，或诱导用户授予与功能无关的敏感权限。
...
请勿把未经授权的真实、精确基站坐标数据提交到公开仓库或随 APK 分发。使用者或二次开发者如替换 `cell_towers.json`，应自行确认数据来源、授权范围、适用地域法规和隐私合规要求，并承担相应责任。
...
- 正式发布 APK 前使用专用 release keystore 签名，不要继续使用 debug 签名。
- 在 GitHub Releases、应用内关于页或发布说明中标明官方发布渠道、版本号和签名信息。
- 源码仓库中的构建产物不自动代表官方发布版本；用户应从开发者声明的渠道下载。
- 不要把签名证书、keystore 密码、API 密钥或其它私密配置提交到仓库。
...
欢迎围绕网络诊断准确性、权限提示、兼容性、界面体验和文档说明提交 Issue 或 Pull Request。涉及以下内容的改动不建议合入：

- 代理、转发、中继、绕过网络限制或隐藏 VPN 行为。
- 任意命令执行、静默 Root、隐藏权限申请或后台持久化。
- 上传小区信息、应用列表、流量统计、位置、设备标识等敏感数据。
- 内置未经授权的真实基站点位库或其它来源不清的数据集。

提交 Issue 或日志时，请不要附带手机号、精确位置、真实基站坐标、账号令牌、证书、密钥或其它个人敏感信息。
